...
interactiveFX <- function(data, spheres=TRUE, color="pitch_types", avg.by,
                          interval=0.01, alpha=1, show.legend=TRUE, ...){
  if (!requireNamespace('rgl'))
      warning("This function requireNamespaces the rgl package. Please try to install.packages('rgl') before using.")
  if ("pitch_type" %in% names(data)) {
    ...
    pitch.types <- c("Sinker", "Fastball (four-seam)", "Intentional Walk", "Slider", "Curveball",
                     "Changeup", "Fastball (two-seam)", "Fastball (cutter)", "Pitchout",
                     "Knuckleball", "Fastball (split-finger)", "Fastball", "Unknown", "Forkball")
    ...
    data <- inner_join(data, types, by = "pitch_type")
  }
  ...
  #get rid of records with any missing parameter values
  complete <- data %>% filter(complete.cases(.[, idx]))
  for (i in idx) complete[,i] <- as.numeric(complete[,i])
  if (!missing(avg.by))
      complete <- complete %>% group_by(avg.by) %>% summarise(mean) %>% as.data.frame
  snaps <- getSnapshots(complete, interval)
  #Number of 'snapshots' requireNamespaced for EVERY pitch to reach home plate
